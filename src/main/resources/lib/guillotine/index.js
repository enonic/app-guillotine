...
function createContext(options) {
    ...
    const creationCallbackBean = __.newBean('com.enonic.app.guillotine.handler.SchemaGetCreationCallbackHandler');
    const creationCallbackFromExtensions = __.toNativeObject(creationCallbackBean.execute()).customCallbacks;
    context.options.creationCallbacks = context.options.creationCallbacks ? Object.create(context.options.creationCallbacks) : {};
    Object.keys(creationCallbackFromExtensions).forEach(key => {
        context.options.creationCallbacks[key] = creationCallbackFromExtensions[key];
    });
