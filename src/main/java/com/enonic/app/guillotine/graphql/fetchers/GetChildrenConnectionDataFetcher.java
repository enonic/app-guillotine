...
import com.enonic.app.guillotine.graphql.ConnectionHelper;
    ...
    extends BaseContentDataFetcher
{
        ...
        throws Exception
    {
        ...
        if ( parentAsMap != null )
        {
            ...
            try
            {
                ...
                return map( children.getTotalHits(), children.getContents().getSize(), offset,
                            children.getContents().stream().map( ContentSerializer::serialize ).collect( Collectors.toList() ) );
            }
            catch ( final ContentNotFoundException e )
            {
                return map( 0, 0, offset, Collections.emptyList() );
            }
        }
        else
        {
    ...
    private Map<String, Object> map( long total, int count, int offset, List<Map<String, Object>> children )
    {
