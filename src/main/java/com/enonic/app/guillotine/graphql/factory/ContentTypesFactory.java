...
public class ContentTypesFactory
{
    ...
    public void create()
    {
        ...
        List<ContentType> contentTypes = serviceFacade.getContentTypeService().getAll().stream().filter(
            contentType -> allowedContentTypesPattern.matcher( contentType.getName().toString() ).find() ).toList();
    ...
    {
        return outputField( "imageUrl", GraphQLTypeReference.typeRef( "ImageUrl" ), imageUrlArguments() );
    }
    ...
    private List<GraphQLFieldDefinition> getGenericContentFields( String contentType )
    {
        ...
        result.add( outputField( "pageUrl", GraphQLTypeReference.typeRef( "PageUrl" ),
