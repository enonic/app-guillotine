...
Queries are executed in the project currently selected in Content Studio and the branch selected in the playground, so the `project` and `branch` arguments of the `guillotine` field are ignored here.
