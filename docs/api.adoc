...
A content outside the site the URL belongs to has no such URL - the site's Base URL does not lead to it - so the field reports an error rather than a URL that resolves to nothing. This can happen when `siteKey` names a site and the query reaches content elsewhere in the project.
