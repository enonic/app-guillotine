...
This section describes the availble GraphQL types. 
...
|Content path. The `type` argument is supported starting from version. In the site-less mode, the result will be returned in the raw value, in other words, `env.source._path`.
