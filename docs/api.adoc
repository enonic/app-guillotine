...
|path: String
|The full media API path with a leading slash, ready to append to a base URL: `/media:image/<context>/<id>:<hash>/<scale>/<name>`
...
|hash: String
|Media fingerprint, joined with the id as `<id>:<hash>` in the path. Derived from the source binary and the image editing data (focal point, cropping, orientation)
...
|path: String
|The full media API path with a leading slash, ready to append to a base URL: `/media:attachment/<context>/<id>:<hash>/<name>`
...
|hash: String
|Media fingerprint derived from the attachment binary, joined with the id as `<id>:<hash>` in the path
