...
|pageUrl(params: Json): String
|Util field to generate a URL pointing to the content

|pageUrlParts(params: Json): <<PageUrlParts>>
|Components of the page URL, for building the full URL from segments
...
|mediaUrl (download: Boolean, type: <<UrlType>>, params: Json): String
|URL to the underlying media attachment. Set `download` to `true` to force download instead of inline rendering.

|imageUrl (scale: String!, quality: Int, type: <<UrlType>>, background: String, format: String, filter: String, params: Json): String
a|URL to a processed version of the image, scaled and cropped by the server at request time.
...
* `type` - URL type: `server` (default) or `absolute`. Use `absolute` when the URL is consumed outside the server context, for instance when rendering from an external frontend or inside Content Studio preview
...
|mediaUrlParts: <<AttachmentUrlParts>>
|Parts of the media URL, honoring the link intent (`?download` for download links)
...
|Parts of the link target page URL. This field has `null` value for media links

|===

=== PageUrlParts

Components of a page URL, for building the full URL from segments: `url = baseUrl + path + queryString`.
...
Parts of an image URL, for building the full URL from segments: `url = baseUrl + path + queryString`. All values are URL-escaped as they appear in the URL.
...
Parts of an attachment URL, for building the full URL from segments: `url = baseUrl + path + queryString`. All values are URL-escaped as they appear in the URL.
...
|attachmentUrl (download: Boolean, params: Json): String
|Attachment URL
