...
|URL pointing to the content, along with its components for building the URL from segments. The URL belongs to the site given as `siteKey`, see <<PageUrl>>
...
|url: String
|The ready-made URL, with the base URL resolved by XP for the site the URL belongs to: the one given as `siteKey`, or - without it - the one the matched virtual host points at when served from a site endpoint, and the nearest site of the content otherwise

A content outside the site the URL belongs to has no such URL - the site's Base URL does not lead to it - so the field reports an error. This can happen when `siteKey` names a site and the query reaches content elsewhere in the project.
...
|url: String
|The URL as the portal library generates it for the current request: the site's configured Base URL or `media.defaultBaseUrl` when `siteKey` is given, the site request with vhost mapping applied when the API is mounted on a site, otherwise server-relative
...
|time: DateTime
|Marker of the `publish.time` property. When present, the draft matches the published version. When absent, the draft has unpublished changes. The value itself carries no meaning
