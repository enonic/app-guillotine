...
|apiUrl: String
|The <<configuration#media-base-url, media base URL>> configured for XP, without a trailing slash, or `null` when none is configured. Neither the request, the site nor its media API mounts plays a part, so it never contains a `/_` segment
