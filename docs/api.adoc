...
|url: String
|The ready-made URL, with the base URL resolved by XP for the site the URL is anchored at (the site given as `siteKey` when present, the site of the content otherwise)

|path: String
|URL-escaped path of the content relative to the site the URL is anchored at (the full content path when it is anchored at the project), with a leading slash. Empty when the content is the anchored site itself
