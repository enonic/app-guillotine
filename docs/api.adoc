...
* Most requests expect a key as an argument. If the key is not specified, then the matched content will be the one corresponding to the current execution context
...
|getSite: portal_Site
|Returns the nearest site for the content ID or path supplied through `guillotine(siteKey: ...)`. Returns `null` when `siteKey` is omitted or selects the project root (`"/"`), including at a site-mounted endpoint. See <<usage/site-context#, Site context>>
...
|Lists built-in content types and content types from installed applications, independently of the selected project, branch or site
|===
...
|_path: String!
|Full content path within the project. For a site-relative link, select `pageUrl { path }` with the appropriate `siteKey`

|_project: String
|Name of the content project used to resolve this item

|_branch: String
|Branch used to resolve this item

|_references: [<<Content>>]
|Content items referenced by this content item (outgoing references)
...
|Page link components. Use `siteKey` to select the site or project that the path is relative to; see <<PageUrl>> and <<usage/images-and-urls#base-urls, Base URLs>>
...
`UntypedContent` is returned for content whose content type is not part of the schema. It provides the `Content` fields without a `data` field.
...
Details of a macro detected in a <<RichText>> value.
...
Dynamic type with one field per macro descriptor. The system macros `disable` and `embed` are always present; other fields are added during schema creation from the macro descriptors of the installed applications.
...
Page link components. Adapt `path` to your frontend's routing and append `queryString`. If your deployment preserves the path, use `frontendBaseUrl + path + queryString`.

|===
|Name (Arguments): Type | Description

|url: String
|Complete page URL as resolved by XP: `baseUrl + path + queryString`. The base is the configured Base URL of the site selected with `siteKey`, or of the content's own site without a selection. A site without a configured Base URL falls back to the site engine address

|path: String
|URL-escaped path of the content relative to the site - or project - the URL belongs to, with a leading slash. Empty when the content is that site itself
...
Image link components. Build the address as `mediaBaseUrl + path + queryString`, using your public API or CDN base URL without a trailing slash. All component values are already URL-escaped. Select `url` to let XP assemble the address instead.

|===
|Name (Arguments): Type | Description

|url: String
|Complete image URL as resolved by XP: `baseUrl + path + queryString`. With `siteKey`, the base is where XP serves the image API for that site, taking API mounts and configuration into account. Without a selection, the URL is relative to the current request
...
Attachment link components. Build the address as `mediaBaseUrl + path + queryString`, using your public API or CDN base URL without a trailing slash. All component values are already URL-escaped. Select `url` to let XP assemble the address instead.

|===
|Name (Arguments): Type | Description

|url: String
|Complete attachment URL as resolved by XP: `baseUrl + path + queryString`. With `siteKey`, the base is where XP serves the attachment API for that site, taking API mounts and configuration into account. Without a selection, the URL is relative to the current request
...
|Typed component config, with one field per application that provides page controllers with a config form. Absent when no installed application does
...
|Typed component config, with one field per application that provides layouts with a config form. Absent when no installed application does
...
|Typed component config, with one field per application that provides parts with a config form. Absent when no installed application does
...
|time: DateTime
|Publication marker. On draft content, its presence indicates that the draft matches the published version; its absence indicates unpublished changes. Use its presence rather than interpreting the timestamp as a publication date
