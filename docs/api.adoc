...
Components of a media URL, for building the full URL from segments: `url = baseUrl + path + queryString`. All values are URL-escaped as they appear in the URL.

|===
|Name (Arguments): Type | Description

|path: String
|The full media API path with a leading slash, ready to append to a base URL: `/media:image/<context>/<id>:<hash>/<scale>/<name>` for images, `/media:attachment/<context>/<id>:<hash>/<name>` for attachments
...
|hash: String
|Media fingerprint, joined with the id as `<id>:<hash>` in the path. For images it is derived from the source binary and the image editing data (focal point, cropping, orientation); for attachments from the attachment binary

|scale: String
|Processed scale segment of image URLs (for example `max-300`). Null for attachments

|name: String
|File name segment. For images the requested format extension is applied

|intent: <<MediaIntentType,MediaIntentType>>
|How the media URL is meant to be consumed: `download` when the URL carries the `?download` query string, `inline` otherwise. Null for image URLs
