...
|Page link components for the site or project selected with `siteKey`, which this field requires; see <<PageUrl>> and <<usage/images-and-urls#base-urls, Base URLs>>
...
Page link components, resolved from configuration for the site or project selected with `siteKey`. The request the query arrives with plays no part. Without `siteKey` the field reports an error, because nothing else says which site the link belongs to. So does a content outside the selected site: the site's Base URL does not lead to it.

Guillotine does not return an assembled address. Build it in the client as `baseUrl + path + queryString`, supplying your frontend's own base when `baseUrl` is `null` or when your deployment routes pages differently.

|===
|Name (Arguments): Type | Description

|baseUrl: String
|The Base URL configured for the selected site or project, without a trailing slash. `null` when none is configured. Configuration is never inherited from a parent site
