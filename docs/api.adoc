...
Image link components. Guillotine does not return an assembled address: build it as `apiUrl + path + queryString`, or use your own public API or CDN base, without a trailing slash, in place of `apiUrl`. All component values are already URL-escaped.

|===
|Name (Arguments): Type | Description

|apiUrl: String
|Where the image API is served, as configured for the site selected with `siteKey`, or for the project without one. It is the site's Base URL followed by `/_` when the API is mounted on the site, and otherwise the <<configuration#media-base-url, media base URL>> configured for XP. `null` when neither is configured. The request plays no part
...
Attachment link components. Guillotine does not return an assembled address: build it as `apiUrl + path + queryString`, or use your own public API or CDN base, without a trailing slash, in place of `apiUrl`. All component values are already URL-escaped.

|===
|Name (Arguments): Type | Description

|apiUrl: String
|Where the attachment API is served, as configured for the site selected with `siteKey`, or for the project without one. It is the site's Base URL followed by `/_` when the API is mounted on the site, and otherwise the <<configuration#media-base-url, media base URL>> configured for XP. `null` when neither is configured. The request plays no part
