...
For media, concatenate your public API or CDN base with `path` and `queryString`. `apiUrl` holds the media base configured in XP, or `null` when there is none. Site mounts of the media APIs are not used, so no URL returned by Guillotine contains a `/_` segment. Links in `processedHtml` follow the same rules: without a configured base they are bare paths, such as `/media:image/...` for images and the site-relative content path for pages. For pages, pass `siteKey`, which `pageUrl` requires, and adapt `pageUrl.path` to your frontend's host and routing, then append `pageUrl.queryString`. `pageUrl.baseUrl` holds the selected site's configured Base URL, or `null` when it has none. These components are already URL-escaped. See <<usage/images-and-urls#url-parts, Building URLs from parts>> for client examples.
...
For page URLs, `siteKey` now selects both the URL base and the relative path, and is required: without it, `pageUrl` produces a GraphQL field error. So does a request for a page URL outside the selected site. For nested sites, choosing the parent or child site can produce different links. Page URLs no longer depend on the endpoint or the request. Test cross-site references and nested-site links against the <<usage/images-and-urls#base-urls, URL context rules>>.
