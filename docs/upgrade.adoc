...
For media, concatenate your public API or CDN base with `path` and `queryString`. For pages, pass `siteKey`, which `pageUrl` requires, and adapt `pageUrl.path` to your frontend's host and routing, then append `pageUrl.queryString`. `pageUrl.baseUrl` holds the selected site's configured Base URL, or `null` when it has none. These components are already URL-escaped. See <<usage/images-and-urls#url-parts, Building URLs from parts>> for client examples.
