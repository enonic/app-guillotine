...
The exported types are documented in <<extending/typescript#, TypeScript>>. Guillotine must also be <<setup#, installed>> in the Enonic instance where you deploy your app.
...
This file must export an `extensions` function returning the definitions and resolvers to add to the schema.

[#first-extension]
== Add your first field

Start with a field that returns a greeting. This example needs no content model or external service.
...
export const extensions = (graphQL: GraphQL): Extensions => ({
    ...
    resolvers: {
        Query: {
            myapp_greeting: (env: DataFetchingEnvironment<{name: string}>): string =>
                `Hello, ${env.args.name}!`,
        },
    },
});
...
Build and deploy your app to the instance running Guillotine. Open <<playground#, Query Playground>> and refresh the schema, or connect a <<usage#third-party-clients, GraphQL client>>. Run:

[source,graphql]
----
{
  myapp_greeting(name: "Ada")
}
----
...
{
  "data": {
    "myapp_greeting": "Hello, Ada!"
  }
}
----

`creationCallbacks` adds the field and its required argument; `resolvers` supplies its value. The field is directly on `Query`, so this example does not need a `guillotine` selection or project and branch arguments. For fields that use content, prefer `HeadlessCms` so resolvers inherit the selected content context; see <<extending/best-practices#, Field placement>>.

If the field is missing, check that your deployed app contains `/guillotine/guillotine.js`, exports `extensions`, and has started successfully. Check the XP logs for extension-loading or schema-generation errors.

== Extension structure

As your extension grows, add the properties you need to the same returned object:
...
The following is a complete alternative for `src/main/resources/guillotine/guillotine.ts`. To keep the greeting example too, merge its `Query` callback and resolver into this extension rather than declaring a second `extensions` function:
