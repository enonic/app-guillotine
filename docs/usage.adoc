...
By default, the `imageUrl`, `mediaUrl` and `attachmentUrl` fields generate server (root-relative) URLs. To generate URLs against a different domain or base URL (for example, a dedicated media host), configure the instance-wide `media.defaultBaseUrl` option in `com.enonic.xp.portal.cfg`, or use the `siteKey` argument with a site that has a configured Base URL.
...
NOTE: With `siteKey`, XP decides where media APIs are served for the site. By default they are implicitly available under the site's `/_` endpoint. If the `legacy.mediaApiAutoMount.enabled` option in `com.enonic.xp.portal.cfg` is disabled, each media URL is anchored to the site's Base URL only when one of the site's applications mounts the corresponding API (`media:image` for images, `media:attachment` for attachments); otherwise it falls back to `media.defaultBaseUrl`, or stays request-based.

=== Building URLs from parts

To build URLs on any origin yourself, use the `pageUrlParts`, `imageUrlParts`, `mediaUrlParts` and `attachmentUrlParts` fields instead of the ready-made URL fields. They return properly URL-escaped segments — including the media fingerprint, which cannot be calculated client-side — so the full URL is a simple concatenation: `url = base + path + queryString`.
    ...
    get(key: "contentid") {
      ...
      ... on media_Image {
        imageUrlParts(scale: "block(800,200)") {
          path       # "/media:image/myproject/contentid:hash/block-800-200/photo.jpg"
          queryString
          hash       # the calculated media fingerprint
        }
      }
    }
  }
...
To generate content (page) links against a different domain or base URL, use the `siteKey` argument with a site (or project) that has a configured Base URL. Content links in `processedHtml` and the value of the `pageUrl` field are then generated against that base URL, followed by the path of the content relative to the site (for example, `https://www.example.com/posts/first-post`).
