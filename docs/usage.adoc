...
----
{
  guillotine {
    get(key: "contentid") {
      pageUrl {
        path         # "/posts/first-post" — escaped, relative to the nearest site
        queryString
      }
      ... on media_Image {
        imageUrl(scale: "block(800,200)") {
...
To generate content (page) links against a different domain or base URL, use the `siteKey` argument with a site (or project) that has a configured Base URL. Content links in `processedHtml` and the `url` value of the `pageUrl` field are then generated against that base URL, followed by the path of the content relative to the site (for example, `https://www.example.com/posts/first-post`).
