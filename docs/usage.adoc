...
Media URLs are generated as the base URL followed by the XP media endpoint path, for example `https://media.example.com/_/media:image/myproject/contentid:hash/block-800-200/photo.jpg` — the media host is expected to serve XP's media API under the `/_/` path (for example, via a vhost mapping `/_/` to `/api/`).
