...
IMPORTANT: The `mediaBaseUrl` argument is disabled by default: any value passed by a client is rejected unless it is listed in the `allowedBaseUrls` <<configuration#, configuration>> option (or that option is set to `*`). The `defaultMediaBaseUrl` configuration option is not affected by this restriction.
...
IMPORTANT: The `pageBaseUrl` argument is disabled by default: any value passed by a client is rejected unless it is listed in the `allowedBaseUrls` <<configuration#, configuration>> option (or that option is set to `*`).
