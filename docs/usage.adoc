...
To generate content (page) links against a different domain or base URL, set the `pageBaseUrl` argument on the `guillotine` field. It is prepended to content links in `processedHtml` and to the value of the `pageUrl` field. The value may be specified with or without a trailing slash. (Media URLs are controlled separately by `mediaBaseUrl`.)
