...
* On the `/api` endpoint there is no site request. With `siteKey`, page URLs use the Base URL configured on the site, and media URLs use the Base URL of the site or the `media.defaultBaseUrl` option of `com.enonic.xp.portal.cfg`. Without a Base URL, URLs are server-relative.
* When the API is mounted on a site, URLs are generated from the site request, and vhost mappings are applied automatically.
...
NOTE: `siteKey` is either the ID of the site content item, or the path to the site within your project. Use `/` to anchor queries to the project root. If no content matches the given `siteKey`, the query fails with an error.
...
Finally, you may retrieve site relative paths - which may be useful when generating site relative URLs for instance. The `path` component of the `pageUrl` field is the URL-escaped path of the content relative to the nearest site, and `_path` is the full content path.
