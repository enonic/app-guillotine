...
NOTE: With `siteKey`, XP decides where media APIs are served for the site. By default they are implicitly available under the site's `/_` endpoint. If the `legacy.mediaApiAutoMount.enabled` option in `com.enonic.xp.portal.cfg` is disabled, media URLs are anchored to the site's Base URL only when one of the site's applications mounts both the `media:image` and `media:attachment` APIs; otherwise they fall back to `media.defaultBaseUrl`, or stay request-based.
