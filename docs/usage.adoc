...
To generate content (page) links against a different domain or base URL, set the `pageBaseUrl` argument on the `guillotine` field. It is prepended to content links in `processedHtml` and to the value of the `pageUrl` field. A trailing slash is appended to the value if missing. An empty value is treated as unspecified. (Media URLs are controlled separately by `mediaBaseUrl`.)
