...
By default, the `imageUrl`, `mediaUrl` and `attachmentUrl` fields generate `server` (relative) URLs. To generate URLs against a different domain or base URL (for example, a dedicated media host), pass the `mediaBaseUrl` argument or set the `mediaBaseUrl` <<configuration#, configuration>> option. The argument takes precedence over the configuration value.
...
----
{
  guillotine {
    get(key: "contentid") {
      ... on media_Image {
        imageUrl(scale: "block(800,200)", mediaBaseUrl: "https://media.example.com/")
      }
    }
  }
}
