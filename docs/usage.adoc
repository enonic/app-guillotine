...
By default, the `imageUrl`, `mediaUrl` and `attachmentUrl` fields generate `server` (relative) URLs. To generate URLs against a different domain or base URL (for example, a dedicated media host), set the `mediaBaseUrl` argument on the `guillotine` field or set the `mediaBaseUrl` <<configuration#, configuration>> option. The argument takes precedence over the configuration value and applies to all media URLs resolved within the query. A trailing slash is appended to the value if missing. An empty value is treated as unspecified.

IMPORTANT: The `mediaBaseUrl` argument is disabled by default: any value passed by a client is rejected unless it is listed in the `allowedBaseUrls` <<configuration#, configuration>> option (or that option is set to `*`). The `mediaBaseUrl` configuration option is not affected by this restriction.
...
By default, all internal links to `media` or `content` will be replaced by a `server` (relative) link in the context of your endpoint.
...
Guillotine by default provides image processing in the `processedHtml` field of the RichText GraphQL type. All internal links to resources will be replaced by a `server` (relative) link and for each `img` tag a `data-image-ref` attribute with referenced content Id will be added.
...
To generate media URLs (images and attachments) in `processedHtml` against a different domain or base URL, set the `mediaBaseUrl` argument on the `guillotine` field or set the `mediaBaseUrl` <<configuration#, configuration>> option. The argument takes precedence over the configuration value. Content (page) links are not affected by `mediaBaseUrl`.
