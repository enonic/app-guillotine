...
With Guillotine setup, you may perform a range of queries. 

NOTE: The API and GrapQL types dynamically adapts to your content model, the examples below are based on a fictional content model and data.

== Content queries

Enonic offers advanced search and aggregation queries using a query DSL (Domain Specific Language). Ranked search, aggregations and highlighting are just some of the features.
