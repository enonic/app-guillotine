...
If the arguments are omitted, the project and branch of the request context are used. When the API is invoked from a site or from Content Studio, the context is defined by the request and the arguments are ignored. See <<setup#endpoints, Endpoints>> for details. The examples below omit the arguments for brevity.
