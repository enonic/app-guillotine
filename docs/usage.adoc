...
By default, the `imageUrl`, `mediaUrl` and `attachmentUrl` fields generate server (root-relative) URLs. To generate URLs against a different domain or base URL (for example, a dedicated media host), set the `mediaBaseUrl` argument on the `guillotine` field, or configure the instance-wide `media.defaultBaseUrl` option in `com.enonic.xp.portal.cfg`. The argument takes precedence and applies to all media URLs resolved within the query. A trailing slash is appended to the value if missing. An empty value is treated as unspecified.

Media URLs are generated as the base URL followed directly by the XP media API path, for example `https://media.example.com/media:image/myproject/contentid:hash/block-800-200/photo.jpg`.

IMPORTANT: The `mediaBaseUrl` argument is disabled by default: any value passed by a client is rejected unless it is listed in the `allowedMediaBaseUrls` <<configuration#, configuration>> option (or that option is set to `*`). Configuration options are not affected by this restriction.
...
To generate media URLs (images and attachments) in `processedHtml` against a different domain or base URL, set the `mediaBaseUrl` argument on the `guillotine` field, or configure the instance-wide `media.defaultBaseUrl` option in `com.enonic.xp.portal.cfg`. The argument takes precedence. Content (page) links are not affected by `mediaBaseUrl`.
