...
This produces media URLs under `https://cdn.example.com/api/media:image/...` and `https://cdn.example.com/api/media:attachment/...` where a more specific API location or a contextual media mount does not take precedence. The same value is reported as `apiUrl` on `ImageUrl` and `AttachmentUrl`. Configure the CDN to forward those paths to XP's corresponding `/api/media:image/...` and `/api/media:attachment/...` endpoints. This setting changes URL generation; the CDN route must be configured separately.
