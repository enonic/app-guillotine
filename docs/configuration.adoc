...
TIP: Base URLs are not configured in Guillotine. A default base URL for media URLs can be configured on XP itself with the `media.defaultBaseUrl` option in `com.enonic.xp.portal.cfg`. When the `siteKey` argument is provided, page URLs and content links in processed HTML belong to the site named by it and start with its configured Base URL, or with `/site/<project>/<branch>/<site path>` when none is configured. Media URLs then use the Base URL of the site or `media.defaultBaseUrl`. When the API is mounted on a site and `siteKey` is not given, URLs follow the site request with vhost mappings applied. See <<usage#url-resolution, URL resolution>>.
